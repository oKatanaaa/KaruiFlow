...
namespace karuiflow {
	...
	std::vector<int> MulCudaKernel::initTensorDescriptor(cutensorTensorDescriptor_t& desc, Storage* storage, Storage* notReduced) {
	...
	void MulCudaKernel::forward(std::vector<Storage*> inputs, Storage* output) {
		...
		// Querry workspace
		spdlog::debug("MulCudaKernel.forward // Quering workspace...");
		...
		spdlog::debug("MulCudaKernel.forward // Workspace acquired.");

		// Perform compute
		spdlog::debug("MulCudaKernel.forward // Computing sum...");
		float alpha = 1.f;
		float beta = 0.f;
		...
		spdlog::debug("MulCudaKernel.forward // Successfully computed sum.");
	}
	...
	void MulCudaKernel::backward(std::vector<Storage*> inputs, std::vector<bool> requiresGrad,
		...
		*/
		...
		default:
			// Impossible case
			throw std::runtime_error("MulCudaKernel.backward // Expected CUDA_R_32F or CUDA_R_32I, but received " + std::to_string(m_CuDtype));
		}
		...
		* ***************************************************/
		...
		float alpha = 1.f;
		float gamma = 1.f;
		spdlog::debug("MulCudaKernel.forward // Computing gradient...");
